...
arg_parser = argparse.ArgumentParser(description='jira_create')
arg_parser.add_argument('--jira_href', type=str, help="--jira_href 'https://jira.ru', default 'https://jira.ru'", default='https://jira.ru')
arg_parser.add_argument('--filein', type=str, help="--filein 'C:/CSV/file.excel'")
...
class jira_creator:
    ...
    def __prepare_time (self, dict_issue):
        ...
        if type(dict_issue) == dict:
            ...
            raise ('Error type ', type(dict_issue))     
    ...
    def __create_mark_issue (self, dict_mark_issue):
        ...
        return id_mark_issue, key_mark_issue
    ...
    def create_project (self):
        ...
        project_key, project_id = self.__create_project_issue(dict_project)
        ...
        for i in list_unique[1:]:
            ...
            mark_id, mark_key = self.__create_mark_issue(dict_marks)
            ...
            for task in dict_tasks:
                print(task)
                dict_tasks = self.__prepare_time(dict_tasks)
                assignee_issue = task.pop('assignee')
                print(task)
                task_key = self.__create_task_issue(task)
